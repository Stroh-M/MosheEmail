import email_handling, os, re, error, datetime, zipfile, traceback, requests, email_utils_beta #type:ignore
...
try:
    ...
    for i in range(len(email_ids)):
        try:
            ...
            found_match = False
            try:
                amazon_orders = email_handling.File(tsv_file_path, 'tsv')

                reader = amazon_orders.read()
                for row_n, row in enumerate(list(reader)):
                    if len(row) > 0:
                        if name.lower().replace('.', '') in email_handling.EmailParser.remove_space_from_middle_of_string(None, row[17].lower().replace('.', '')) and zip in row[23]:
                            found_match_amazon = True
                            amazon_order_id = row[0]

                            data = []
                            carrier = email_utils_beta.get_carrier(tracking_number=tracking[0])
                            ship_date = datetime.datetime.now().date()

                            data.append([(1, amazon_order_id), (4, ship_date), (5, carrier), (7, tracking[0])])

                            try:
                                a_s = email_handling.File(excel_file_path, 'xlsx', sheet_name)
            ...
            if not found_match_amazon:
                try:
                    w_s = email_handling.File(walmart_order_excel_file, 'xlsx', sheet='Po Details')
                    data = []
                    ...
                    for idx, row in enumerate(w_s.iter_rows(), start=1):
                        if zip == row[13] and name == email_handling.EmailParser.remove_space_from_middle_of_string(None, row[5]):
                            found_match = True
            ...
            if not found_match:
                ...
                try:
                    e_s = email_handling.File(error_excel_path, 'xlsx')
...
except email_handling.imaplib.IMAP4_SSL.error as mail_s_e:
